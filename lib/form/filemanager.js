...
M.form_filemanager.init = function (Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        update_file: function (confirmed) {
            ...
            if (fileinfo.type == 'folder') {
                ...
                if (filenamechanged || filepathchanged || licensechanged || authorchanged) {
                    params = {
                        filepath: fileinfo.filepath, filename: fileinfo.fullname,
        ...
        setup_header_buttons: function () {
            var bbox = this.selectui.get('boundingBox');
            var headerButtons = bbox.one('.yui3-widget-buttons');
            if (headerButtons && !headerButtons.one('.fp-header-download')) {
                var styleId = 'fp-header-buttons-styles';
                if (!Y.one('#' + styleId)) {
                    var css = '.fp-header-btn { background: transparent; border: none; padding: 0.5rem; margin-right: 0.25rem; font-size: 1.1rem; color: #6c757d; cursor: pointer; transition: color 0.15s ease-in-out; vertical-align: middle; } ' +
                              '.fp-header-btn:hover { color: #343a40; text-decoration: none; } ' +
                              '.fp-header-delete:hover { color: #dc3545; }';
                    var styleNode = Y.Node.create('<style id="' + styleId + '">' + css + '</style>');
                    Y.one('head').append(styleNode);
                }

                var selectnode = this.selectnode;

                var downloadBtn = Y.Node.create(
                    '<button class="fp-header-btn fp-header-download" ' +
                    'title="' + M.util.get_string('download', 'moodle') + '">' +
                    '<i class="fa fa-download"></i></button>'
                );
                var deleteBtn = Y.Node.create(
                    '<button class="fp-header-btn fp-header-delete" ' +
                    'title="' + M.util.get_string('delete', 'moodle') + '">' +
                    '<i class="fa fa-trash"></i></button>'
                );

                headerButtons.prepend(deleteBtn);
                headerButtons.prepend(downloadBtn);

                downloadBtn.on('click', function (e) {
                    e.preventDefault();
                    var origDownload = selectnode.one('.fp-file-download');
                    if (origDownload) {
                        origDownload.simulate('click');
                    }
                });

                deleteBtn.on('click', function (e) {
                    e.preventDefault();
                    var origDelete = selectnode.one('.fp-file-delete');
                    if (origDelete) {
                        origDelete.simulate('click');
                    }
                });
            }
        },
        ...
        select_file: function (node) {
            ...
            var bbox = this.selectui.get('boundingBox');
            var downloadBtn = bbox.one('.fp-header-download');
            if (downloadBtn) {
                if (node.type == 'folder') {
                    downloadBtn.hide();
                } else {
