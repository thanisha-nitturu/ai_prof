...
class completion_info {
    ...
    public function get_activities() {
        ...
        $result = array();
        foreach ($modinfo->get_cms() as $cm) {
            //this include hidden and stealth activities
            // if ($cm->completion != COMPLETION_TRACKING_NONE && !$cm->deletioninprogress) {
            //     $result[$cm->id] = $cm;
            // }
            //this doesnt include hidden
            if ($cm->completion != COMPLETION_TRACKING_NONE && !$cm->deletioninprogress && $cm->visible) {
                $result[$cm->id] = $cm;
            }
        }
        return $result;
    ...
    public function count_modules_completed(int $userid): int {
        ...
        // IMPORTANT: Must use same visibility filters as get_activities() (the denominator).
        // Without cm.visible = 1, hidden modules (visible=0) such as hidden quizzes or
        // reengagement modules get counted in the numerator but NOT the denominator,
        // causing course progress to exceed 100%.
        // Stealth modules (visible=1, visibleoncoursepage=0) are correctly included in both.
        $sql = "SELECT COUNT(1)
                  FROM {course_modules} cm
