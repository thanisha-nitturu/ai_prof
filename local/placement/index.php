...
// $PAGE->set_heading('Placement Dashboard');
$PAGE->set_primary_active_tab('placement');

$PAGE->add_body_class('placement-fullwidth');
$PAGE->requires->css(new moodle_url('/local/placement/fullwidth.css'));
...
echo "window.PLACEMENT_CONFIG = " . json_encode([
    'userId' => $USER->id,
    'userName' => fullname($USER),
    'userEmail' => $USER->email,
    'sesskey' => sesskey(),
    'apiUrl' => $CFG->wwwroot . '/local/placement/api.php',
    'userRole' => $userrole,
]) . ";";
...
echo html_writer::div('', '', ['id' => 'placement-root']);

// Load CSS and JS bundles
$pluginurl = new moodle_url('/local/placement/static/build');
...
if (is_readable($jsfile)) {
    ...
    echo html_writer::div(
        'Placement bundle not found or not built. Please build or deploy the static client files.',
        'alert alert-danger'
    );
}
