    ...
    public function get_player() {
        ...
        if ($this->supervideo->videourl) {
            ...
            if ($this->supervideo->origem == "link") {
                ...
                $isaudio  = (bool)preg_match("/^https?.*\.(mp3|aac|m4a)/i", $videourl);

                // Build VideoJS data-setup-lazy config — same as media_videojs_plugin::embed() does.
                $datasetup = [
                    '"fluid": true',
                    '"playbackRates": [0.5, 0.75, 1, 1.25, 1.5, 1.75, 2]',
                    '"userActions": {"hotkeys": true}',
                ];
                if ($this->supervideo->autoplay) {
                ...
                if ($this->supervideoview->currenttime > 0) {
                    $datasetup[] = '"startTime": ' . (int)$this->supervideoview->currenttime;
                }

                $attrs = [
                    'id'             => $elementid,
                    'class'          => 'video-js vjs-default-skin',
                    'data-setup-lazy'=> '{' . implode(', ', $datasetup) . '}',
                    'preload'        => 'auto',
                    'crossorigin'    => 'anonymous',
                    'playsinline'    => 'true',
                ];
                if ($this->supervideo->showcontrols) {
                    $attrs['controls'] = 'true';
                }

                $source = html_writer::empty_tag('source', ['src' => $videourl]);
                $tag    = $isaudio ? 'audio' : 'video';
                $inner  = html_writer::tag($tag, $source, $attrs);
                $html   = html_writer::div($inner, 'mediaplugin mediaplugin_videojs d-block');

                $PAGE->requires->js_call_amd(
                    'mod_supervideo/player_create',
                    $isaudio ? 'resource_audio' : 'resource_video',
                    [
                        (int)$this->supervideoview->id,
                        $this->supervideoview->currenttime,
                        $elementid,
            ...
            if ($this->supervideo->origem == "upload") {
                ...
                if ($file) {
                    ...
                    $isaudio = in_array($extension, ['mp3', 'aac', 'm4a']);
                    ...
                    $source = html_writer::empty_tag('source', ['src' => $fullurl]);
                    $tag    = $isaudio ? 'audio' : 'video';
                    $inner  = html_writer::tag($tag, $source, $attrs);
                    $html   = html_writer::div($inner, 'mediaplugin mediaplugin_videojs d-block');

                    // Register tracking hook — just attach to the existing VideoJS player.
                    $PAGE->requires->js_call_amd(
                        'mod_supervideo/player_create',
                        $isaudio ? 'resource_audio' : 'resource_video',
                        [
