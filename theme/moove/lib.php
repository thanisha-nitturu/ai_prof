...
/**
 * Reorder primary navigation items.
 *
 * @param navigation_node $nav The navigation node to extend.
 */
function theme_moove_extend_navigation_primary(navigation_node $nav) {
    ...
    // 3. Re-add them in the order: Home -> My courses -> Calender (Dashboard).
    if ($home) {
        $nav->add_node($home);
    }
    if ($mycourses) {
