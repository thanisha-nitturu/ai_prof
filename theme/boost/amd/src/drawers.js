...
import { getElement as getFullscreenElement } from 'core/fullscreen';
...
export default class Drawers {
    ...
    closeDrawer({ focusOnOpenButton = true, updatePreferences = true } = {}) {
        window.console.warn('[DrawerDebug] closeDrawer called for:', this.drawerNode.id, {
            isOpen: this.isOpen,
            isSmall: isSmall(),
            isFullscreen: !!getFullscreenElement(),
            isFullscreenTransition: isFullscreenChange
        });
...
const registerListeners = () => {
    ...
    document.addEventListener('click', e => {
        ...
        if (closeDrawerButton && closeDrawerButton.dataset.target) {
            e.preventDefault();
            window.console.warn('[DrawerDebug] Manual close button clicked for:', closeDrawerButton.dataset.target);
    ...
    const closeOnResizeListener = () => {
        if (isFullscreenChange || getFullscreenElement()) {
            return;
        }
        if (isSmall()) {
            let anyOpen = false;
            drawerMap.forEach(drawerInstance => {
                ...
                if (drawerInstance.isOpen) {
                    ...
                    if (shouldClose) {
                        window.console.warn('[DrawerDebug] Auto-closing drawer via closeOnResize:', drawerInstance.drawerNode.id);
                        drawerInstance.closeDrawer();
                    } else {
                        anyOpen = true;
                    }
