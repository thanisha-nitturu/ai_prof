...
export default class Component extends BaseComponent {
    ...
    //         // We consider the element active if it reaches the top of the page.
    //         // We use a small threshold to account for fixed headers.
    //         if (rect.top <= 50) {
    //             pageItem = item;
    ...
    _scrollHandler() {
        const items = this.reactive.getExporter().allItemsArray(this.reactive.state);
        // We use a larger offset to account for fixed headers and scroll margins.
        const HEADER_OFFSET = 150;

        let pageItem = null;

        // Find the last item that has crossed the header threshold.
        items.every(item => {
            const index = (item.type === 'section') ? this.sections : this.cms;
            if (index[item.id] === undefined) {
                return true;
            ...
            if (rect.height === 0 || element.offsetParent === null) {
                return true;
            }

            // We consider the element active if it reaches or crosses the threshold
